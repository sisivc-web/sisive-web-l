...
import {kpiApplicationDocument, kpiVisaApplicationPost, kpiVisaApplicationPut, kpiCodeCountry} from 'apx'
export default class OverseasStudyApply {
  constructor(details, orderId, itemId) {
    ...
    this.visaType = details ? details.visaType : ''
  ...
  save() {
    let _this = Vue.prototype
    kpiVisaApplicationPost(this, _this).then((res) => {
      console.log('签证申请: ', res)
      _this.$toast('保存成功!', {
        complete: () => {
  ...
  submit() {
    ...
    kpiVisaApplicationPost(this, _this).then((res) => {
      ...
      kpiVisaApplicationPut(param, _this).then((res) => {
        console.log('已提交: ', res)
        _this.$toast('提交成功!', {
          complete: () => {
            router.replace('/personalCenter/adviserApplys?activedIndex=0')
